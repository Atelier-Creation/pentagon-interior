...
export default function EcoLiving() {
  const [hoveredIndex, setHoveredIndex] = useState(null);
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 });
  
  const sidebar = content.blogSidebar;
  const currentPost = content.blogs.find(b => b.path === "/blog/eco") || content.blogs[4];
      ...
      <section className="max-w-7xl mx-auto px-4 py-16 grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
        ...
        <div className="lg:col-span-2">
          ...
          <p className="text-gray-600 leading-7 mb-6 mt-10">
            {currentPost.title} focuses on sustainable practices and eco-friendly choices for a modern home. It's about reducing environmental impact while maintaining style and comfort.
          </p>
          ...
          <p className="text-gray-600 mb-4 leading-7">
            Start with sustainable materials like bamboo, recycled wood, and organic fabrics to create a healthy living environment.
          </p>
          <ul className="space-y-3 text-gray-600 mb-8">
            {["Use reclaimed wood for furniture.", "Choose organic cotton or linen fabrics.", "Opt for bamboo flooring and accessories."].map((item, i) => (
          ...
          <p className="text-gray-600 mb-4 leading-7">
            Reduce your carbon footprint by incorporating energy-efficient lighting and smart home systems.
          </p>
          <ul className="space-y-3 text-gray-600 mb-8">
            {["Install LED lighting throughout the house.", "Use smart thermostats to control energy usage.", "Maximize natural ventilation to reduce cooling needs."].map((item, i) => (
              <li key={i} className="flex items-start gap-3">
                <CheckCircle2 size={16} className="mt-1 text-[#8c6d5a]" />
                {item}
              </li>
            ...
            <div className="relative flex flex-col sm:flex-row items-center gap-4 md:gap-6 p-10 text-center sm:text-left text-white">
              ...
              <div>
                <p className="text-xl md:text-2xl font-semibold leading-relaxed">
                  Sustainable design is not just a trend, it's a responsibility towards our future.
                </p>
          ...
          <p className="text-gray-600 mb-6 leading-7">
            Implement simple waste reduction strategies like composting and choosing products with minimal packaging.
          </p>
          <div className="space-y-6 mb-10">
            {[
              { title: "Reduce", desc: "Buy only what you need and choose quality over quantity." },
              { title: "Reuse", desc: "Repurpose old items instead of throwing them away." },
              { title: "Recycle", desc: "Properly sort and recycle waste materials." }
            ].map((item, i) => (
          ...
          <p className="text-gray-600 mb-6 leading-7">
            Eco-friendly living is about making conscious choices that benefit both you and the planet.
          </p>
          <ul className="space-y-4 text-gray-600 mb-10">
            {["Small changes in daily habits can lead to a significant environmental impact.", "Investing in sustainability ensures a healthier home for your family.", "Sustainable living promotes a sense of harmony and well-being."].map((item, i) => (
              <li key={i} className="flex items-start gap-3">
