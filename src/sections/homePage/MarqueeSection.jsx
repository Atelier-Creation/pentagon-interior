...
const MarqueeSection = () => {
  ...
  return (
    <section className="py-20 bg-white overflow-hidden relative flex items-center group cursor-pointer selection:bg-transparent">
      
      {/* Center Dashed Frame Guide Overlay */}
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-[52%] w-[95%] max-w-[1255px] h-[75px] lg:h-[125px] border border-dashed border-[#cbd8cb]/70 pointer-events-none z-10 hidden md:block"></div>
      ...
      <div className="flex w-max animate-marquee group-hover:[animation-play-state:paused] relative z-20">
        ...
        {displayItems.map((item, i) => (
          <div key={i} className="group/item relative flex items-center shrink-0 px-8">
             ...
             <span 
               className="text-[80px] lg:text-[130px] font-sans font-black uppercase transition-all duration-300 text-transparent group-hover/item:text-[#e4ede4]" 
               style={{ WebkitTextStroke: '2px #cbd8cb', paddingBottom: '10px' }}
             >
               {item.text}
             </span>
             
             {/* The Separator Star */}
             <svg className="w-12 h-12 lg:w-16 lg:h-16 mx-10 text-[#cbd8cb] group-hover/item:text-[#e4ede4] transition-colors" viewBox="0 0 24 24" fill="currentColor">
